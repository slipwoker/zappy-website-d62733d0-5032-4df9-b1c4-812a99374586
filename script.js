  ...
  if (contactForm) {
    const showError = function(input, message) {
      ...
      if (!errorEl) {
        ...
        errorEl.style.cssText = 'color:#dc3545;font-size:0.875rem;margin-top:0.25rem;display:block;';
    ...
    const validateField = function(input) {
      ...
      if (input.hasAttribute('required') && value === '') {
        showError(input, isRTL ? 'هذا الحقل مطلوب' : 'This field is required');
        return false;
      }
      
      if (input.type === 'email' && value !== '' && !validateEmail(value)) {
        showError(input, isRTL ? 'يرجى إدخال بريد إلكتروني صالح' : 'Please enter a valid email address');
        return false;
      }
      
      if (input.hasAttribute('minlength') && value.length < parseInt(input.getAttribute('minlength'))) {
        const min = input.getAttribute('minlength');
        showError(input, isRTL ? `يجب أن يحتوي على ${min} أحرف على الأقل` : `Must be at least ${min} characters`);
        return false;
      }
    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      if (isValid) {
        ...
        if (submitBtn) {
          ...
          submitBtn.textContent = document.documentElement.dir === 'rtl' ? 'جارٍ الإرسال...' : 'Sending...';
          
          setTimeout(function() {
            submitBtn.textContent = document.documentElement.dir === 'rtl' ? 'تم الإرسال!' : 'Sent!';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'd62733d0-5032-4df9-b1c4-812a99374586',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'd62733d0-5032-4df9-b1c4-812a99374586';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd62733d0-5032-4df9-b1c4-812a99374586';
